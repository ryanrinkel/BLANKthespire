...
ONE minion at a time, so `summon` is the class's setup card (HP 8-15 at 1-2 cost; re-summoning raises Max HP) and the payoff is summon_attack (per-hit damage BELOW a same-cost Strike, since the pet's Strength scales it). buff_summon Strength at +1/+2; heal_summon/shield_summon in the 3-6 band, and at most one per-turn medic power. Never put a summon on a basic, and never stack a summon-buff engine on top of a player-Strength engine. The pet's hits count as YOU dealing damage, so `on_damage_dealt` + summon_attack is the class's pack-tactics engine (once_per_turn; small reward). A class may declare TWO minions and opt ONE into the AUTONOMOUS model (v52): a per-turn `moves` cycle is a POWER that costs a card, so price its output at 4-6 damage or Block per turn (hard cap 8 damage / 6 Block) and give it 10-20 HP, well under a bodyguard's. An ETHEREAL striker (`attackable`: false) never body-blocks and gets no heal/shield value, so it takes the low end of the HP band and the high end of the damage band. `sacrifice_summon` is a card plus the minion's remaining value: the payoff on that card must be >= 10 Block, 12 damage, or 2 draws + 1 energy, and it shines when the minion carries an `on_death` rattle (sacrifice it on purpose, then re-summon).
