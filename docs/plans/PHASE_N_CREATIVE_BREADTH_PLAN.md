...
| **O-2a** | hybrid-kinds RESEARCH GATE (C# read + hand-staged smoke) | — | **PASS 2026-09-11** (VOCAB_GAP_REMEDIATION_PLAN Phase AW): AutoSlay `GAPTESTAW1` drove a hand-built orb+status hybrid (`generation/scratch/gaptest-aw/build_tester.py`: 3 orb slots, Plasma/Cinder + Scorch/Kindle/Ashen) through 37 rooms into Act 3 — 377 `[AR]` orb tags (all six gated shapes, OPEN and closed) + 1,246 `[AQ]` status tags (all three hooks) in ONE run, 0 mod-attributable exception frames; the orb HUD and the custom status icons coexist. Previously: done 2026-07-08 — PARTIAL PASS. (1) C# read: ForgedCharacters.TryParse (:180-210) parses orb_pool/status_pool/summon_pool INDEPENDENTLY and CharacterSpec carries all three — NO mutual-exclusivity gate; ForgedCards SupportedOps holds both orb ops + apply_status_custom with no parse-time class-kind XOR. So the IMPORT side accepts hybrids by construction. (2) Hand-staged a minimal hybrid to slot 04 ('Hybrid Smoke O2a': orb_slots 3 + orb_pool [lightning] + status_pool [Focus Edge], 5 cards incl channel/evoke/apply_status_custom), built, AutoSlay seed GAPTESTO2 --character class4. Result: **import PASS** (`class 04 <- 'Hybrid Smoke O2a' (HP 75, deck 5)`, all 5 cards registered, **0 mod exceptions**), **embark PASS** (`Embarking on a singleplayer ...SLOT04 run`, no orb/status runtime exception). **Combat HUD-coexistence + both-mechanics-firing UNVERIFIED** — the AutoSlay driver stalled at `Room type not assigned` (env map-stall in MegaCrit's AutoSlay helper, BEFORE any room/combat; the earlier N-ACC run today hit the same class of env watchdog). Not a hybrid defect — the game accepted + embarked the hybrid; the env just can't drive combat today. Backup/restore of slot 04 done. |
| **O-2b** | hybrid class kinds in generation | O-2a PASS | **done 2026-09-11 as VOCAB_GAP_REMEDIATION_PLAN Phase AW** (see its STATUS paragraph): `Candidate.class_kinds` (primary + ONE splash, `catalog.candidate_kinds`, cap 2), the dossier brief's HYBRID sentence, `_validate_hybrid` splash budgets (splash orb = 2-3 slots + ≤1 custom orb / splash status ≤2 / splash summon = ONE passive minion; never three pools), both pools' exemplars + prompt sections dealt, `_KIND_WEIGHT` hybrid 3.5, fake path grafts the splash. The `splash_kind` compose field sketched below was NOT needed — the kind set falls out of the fused archetypes' class_kinds. Previously: SKIPPED 2026-07-08 (§0.5) — O-2a proved import+embark but the runtime HUD/mechanic coexistence is UNVERIFIED (AutoSlay env-blocked). Conservatively do NOT build hybrid GENERATION on unverified runtime support; revisit when AutoSlay can drive a hybrid to combat (or a manual combat playthrough) confirms the orb HUD + custom-status icons coexist and both fire. Phase O ships without hybrids, as §0.5 anticipates. |
